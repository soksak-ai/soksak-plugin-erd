...
| 레이어 | 소유물 | 신설 여부 |
|---|---|---|
| **사이드카** `soksak-sidecar-erd-db` (신규 repo, Rust) | DB 드라이버·커넥션 풀·다이얼렉트별 introspection 쿼리·SQL 실행·**신뢰경계(read-only role 검증)**·결과 마스킹·감사 원장·`.mig` ledger·**destructive 확인 권위(PendingConfirms)** | **신설** — 유일한 새 바이너리 |
...
- 비밀번호는 vault(`ns=soksak-plugin-erd`)의 키 **`env:ERD_DB_<profileId>_PASSWORD`** 로 저장. **`secret.get` 명령 부재**(`catalogSecrets.ts`) — 평문 readback 원천 차단.
...
| `db-profile-secret` | plugin | `{profile, password}` | 비밀번호를 vault `env:ERD_DB_<id>_PASSWORD` 봉인. readback 없음 | 값 스크러빙 (§4-note) |
...
## 8. 사용자 필수 결정 (v1 착수 전 차단 결정)

> 비평 검증: 아래는 "연기 가능 미결"이 아니라 **v1 설계를 확정하려면 먼저 정해야 하는** 정책이다.

**A. 게이트 실체 정정 방향** — §2 의 `remoteDestructive` 기본 `allow` 를 어떻게 다룰지: (1) 사이드카 PendingConfirms+`remote.confirm` 를 destructive 의 유일 경로로(권장), (2) 설치 시 `remoteDestructive:"deny"` 강제+고지, (3) 둘 다.
**B. 기본 read-only 강도** — 신규 프로필은 read 프로필만으로 시작하고 admin 은 별도 명시 등록 요구(권장) vs 한 번에 둘 등록 허용.
**C. prod write 전면 차단 여부** — 현 설계는 등급3 만 prod 무조건 거부. Supabase 권고("프로덕션 에이전트 직결 금지") 따라 prod 에서 `db-exec` 까지 전면 차단(migration-run 만)할지 — 운영 hotfix 워크플로와 충돌 여지.
**D. environment 신뢰 모델** — 자기신고 문자열 수용 vs 서버 지문 검증(§4-note 는 후자 전제).
**E. `readonly_verified:false` 접속의 read** — write 게이트 상향 시 매 쿼리 승인=approval fatigue. "최초 1회 세션 승인 후 자동허가"로 완화 vs 매 쿼리 게이트.
...
- **Phase 1 — 사이드카 스캐폴드** — `soksak-sidecar-erd-db` repo `git init`+초기 커밋(신규 플러그인 규율). `soksak-spec-service` `serve()` 차용, `ping`/`db-connect`/`db-disconnect`/`db-status`. 드라이버 1종(SQLite 먼저)+커넥션 풀. RED: `db-test` 로 실 SQLite 파일 접속·버전 반환.
...
- **Phase 8 — 멀티다이얼렉트·배포** — MySQL/PG 드라이버 추가, 5플랫폼 사이드카 빌드+sha256 핀+immutable-forward 발행. `soksak-erd` SKILL.md 동시 갱신(스킬 drift 방지).
